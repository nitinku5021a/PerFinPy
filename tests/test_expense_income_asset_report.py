...
def test_expense_income_asset_report_grouping():
    db_path = os.path.join(
        os.getcwd(),
        "tmp",
        f"expense-income-asset-{uuid4().hex}.db",
    )
    ...
    with app.app_context():
        ...
        breakdown_options = {item["short_label"]: item for item in payload["breakdown_options"]}
        salary_key = breakdown_options["Salary"]["key"]
        rent_key = breakdown_options["Rent"]["key"]
        ...
        assert months["2023-12"]["breakdowns"][salary_key] == 700.0
        assert months["2023-12"]["breakdowns"][rent_key] == 200.0
        ...
        assert years[2023]["breakdowns"][salary_key] == 700.0
        assert years[2023]["breakdowns"][rent_key] == 200.0
        ...
        assert years[2024]["breakdowns"][salary_key] == 2200.0
        assert years[2024]["breakdowns"][rent_key] == 1000.0
