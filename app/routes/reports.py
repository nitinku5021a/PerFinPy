...
@bp.route('/financial-freedom-clock')
def financial_freedom_clock():
    payload = reports_service.financial_freedom_clock_cached_report()
    return jsonify(payload)

@bp.route('/financial-freedom-clock/refresh', methods=['POST'])
def financial_freedom_clock_refresh():
    target_months = request.args.get('target_months')
    if target_months is None and request.is_json:
        target_months = request.json.get('target_months')
    if target_months:
        try:
        ...
        target_months = 300
    payload = reports_service.refresh_financial_freedom_clock_report(target_months)
    return jsonify(payload)

@bp.route('/dashboard-panels')
def dashboard_panels():
    payload = reports_service.dashboard_panels_cached_report()
    return jsonify(payload)

@bp.route('/dashboard-panels/refresh', methods=['POST'])
def dashboard_panels_refresh():
    target_months = request.args.get('target_months')
    if target_months is None and request.is_json:
        target_months = request.json.get('target_months')
    if target_months:
        ...
        target_months = 300
    payload = reports_service.refresh_dashboard_panels(target_months)
    return jsonify(payload)
