...
def export_transactions(period):
    ...
    freedom_clock_ws = wb.create_sheet('Financial Freedom Clock')
    freedom_clock_ws.append([
        'Target Months',
        'Net Worth',
        'Monthly Expenses',
        'Monthly Saving',
        'Net Worth Change Last Month',
        'Months Funded',
        'Saving Coverage %',
        'Days Freedom Gained',
        'Freedom Progress %',
    ...
    dashboard_nw_ws = wb.create_sheet('Dashboard Net Worth Monthly')
    dashboard_nw_ws.append([
        'Month',
        'Net Worth',
        'Real Estate',
        'Liquid Net Worth',
    ...
    dashboard_savings_ws = wb.create_sheet('Dashboard Net Savings Series')
    dashboard_savings_ws.append([
        'Month',
        'Income',
        'Expense',
        'Net Savings',
    ...
    if freedom_snapshot:
        freedom_clock_ws.append([
            int(freedom_snapshot.target_months or 300),
            float(freedom_snapshot.net_worth or 0.0) if freedom_snapshot.net_worth is not None else '',
            float(freedom_snapshot.monthly_expenses or 0.0) if freedom_snapshot.monthly_expenses is not None else '',
            float(freedom_snapshot.monthly_saving or 0.0) if freedom_snapshot.monthly_saving is not None else '',
            float(freedom_snapshot.net_worth_change_last_month or 0.0) if freedom_snapshot.net_worth_change_last_month is not None else '',
            float(freedom_snapshot.months_funded or 0.0) if freedom_snapshot.months_funded is not None else '',
            float(freedom_snapshot.saving_coverage_pct or 0.0) if freedom_snapshot.saving_coverage_pct is not None else '',
            float(freedom_snapshot.days_freedom_gained or 0.0) if freedom_snapshot.days_freedom_gained is not None else '',
            float(freedom_snapshot.freedom_progress_pct or 0.0) if freedom_snapshot.freedom_progress_pct is not None else '',
            freedom_snapshot.updated_month.strftime('%Y-%m') if freedom_snapshot.updated_month else '',
            freedom_snapshot.updated_at.strftime('%Y-%m-%d %H:%M:%S') if freedom_snapshot.updated_at else '',
            bool(freedom_snapshot.has_data)
        ])

    cache_entries = DashboardPanelCache.query.all()
    cache_map = {entry.key: entry for entry in cache_entries}

    networth_cache = cache_map.get('networth_monthly')
    savings_cache = cache_map.get('net_savings_series')

    if networth_cache:
        try:
            payload = json.loads(networth_cache.payload_json or '{}')
        except Exception:
            payload = {}
        for row in payload.get('months', []) or []:
            dashboard_nw_ws.append([
                row.get('month') or '',
                row.get('networth') if row.get('networth') is not None else '',
                row.get('real_estate') if row.get('real_estate') is not None else '',
                row.get('liquid_networth') if row.get('liquid_networth') is not None else '',
                row.get('delta') if row.get('delta') is not None else '',
                row.get('pct_change') if row.get('pct_change') is not None else ''
            ])

    if savings_cache:
        try:
            payload = json.loads(savings_cache.payload_json or '{}')
        except Exception:
            payload = {}
        for row in payload.get('months', []) or []:
            dashboard_savings_ws.append([
                row.get('month') or '',
                row.get('income') if row.get('income') is not None else '',
                row.get('expense') if row.get('expense') is not None else '',
                row.get('net_savings') if row.get('net_savings') is not None else '',
                row.get('net_savings_pct') if row.get('net_savings_pct') is not None else ''
            ])
