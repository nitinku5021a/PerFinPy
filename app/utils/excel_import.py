...
def import_transactions_from_excel(file_stream):
    ...
    try:
        ...
        # Import Financial Freedom Clock snapshot, if present
        if 'Financial Freedom Clock' in workbook.sheetnames:
            fc_ws = workbook['Financial Freedom Clock']
            for row_idx, row in enumerate(fc_ws.iter_rows(min_row=2, values_only=True), start=2):
                if not row or not any(row):
                    continue
                try:
                    target_months = int(row[0] or 300)
                    net_worth = float(row[1]) if row[1] not in (None, '') else None
                    monthly_expenses = float(row[2]) if row[2] not in (None, '') else None
                    monthly_saving = float(row[3]) if row[3] not in (None, '') else None
                    net_worth_change = float(row[4]) if row[4] not in (None, '') else None
                    months_funded = float(row[5]) if row[5] not in (None, '') else None
                    saving_coverage = float(row[6]) if row[6] not in (None, '') else None
                    days_freedom = float(row[7]) if row[7] not in (None, '') else None
                    freedom_progress = float(row[8]) if row[8] not in (None, '') else None
                    updated_month = _parse_month_cell(row[9] if len(row) > 9 else None)
                    updated_at = _parse_datetime_cell(row[10] if len(row) > 10 else None)
                    has_data = _parse_bool_cell(row[11] if len(row) > 11 else None, default=False)
                    ...
                    snapshot.target_months = target_months
                    snapshot.net_worth = net_worth
                    snapshot.monthly_expenses = monthly_expenses
                    snapshot.monthly_saving = monthly_saving
                    snapshot.net_worth_change_last_month = net_worth_change
                    snapshot.months_funded = months_funded
                    snapshot.saving_coverage_pct = saving_coverage
                    snapshot.days_freedom_gained = days_freedom
                    snapshot.freedom_progress_pct = freedom_progress
                    snapshot.updated_month = updated_month
                    snapshot.has_data = bool(has_data)
                    if updated_at:
                        snapshot.updated_at = updated_at
                except Exception as e:
                    results['warnings'].append(f"Financial Freedom Clock row {row_idx}: {str(e)}")

        # Import Dashboard Net Worth Monthly cache, if present
        if 'Dashboard Net Worth Monthly' in workbook.sheetnames:
            nw_ws = workbook['Dashboard Net Worth Monthly']
            months = []
            for row_idx, row in enumerate(nw_ws.iter_rows(min_row=2, values_only=True), start=2):
                if not row or not any(row):
                    continue
                try:
                    month = str(row[0] or '').strip()
                    if not month:
                        raise ValueError('Month is required')
                    months.append({
                        'month': month,
                        'networth': float(row[1]) if row[1] not in (None, '') else None,
                        'real_estate': float(row[2]) if row[2] not in (None, '') else None,
                        'liquid_networth': float(row[3]) if row[3] not in (None, '') else None,
                        'delta': float(row[4]) if row[4] not in (None, '') else None,
                        'pct_change': float(row[5]) if row[5] not in (None, '') else None
                    })
                except Exception as e:
                    results['warnings'].append(f"Dashboard Net Worth row {row_idx}: {str(e)}")

            if months:
                payload = {'months': months}
                cache = DashboardPanelCache.query.filter_by(key='networth_monthly').first()
                if cache is None:
                    cache = DashboardPanelCache(key='networth_monthly', payload_json=json.dumps(payload))
                    db.session.add(cache)
                else:
                    cache.payload_json = json.dumps(payload)

        # Import Dashboard Net Savings Series cache, if present
        if 'Dashboard Net Savings Series' in workbook.sheetnames:
            savings_ws = workbook['Dashboard Net Savings Series']
            months = []
            for row_idx, row in enumerate(savings_ws.iter_rows(min_row=2, values_only=True), start=2):
                if not row or not any(row):
                    continue
                try:
                    month = str(row[0] or '').strip()
                    ...
                    months.append({
                        'month': month,
                        'income': float(row[1]) if row[1] not in (None, '') else None,
                        'expense': float(row[2]) if row[2] not in (None, '') else None,
                        'net_savings': float(row[3]) if row[3] not in (None, '') else None,
                        'net_savings_pct': float(row[4]) if row[4] not in (None, '') else None
                    })
                except Exception as e:
                    results['warnings'].append(f"Dashboard Net Savings row {row_idx}: {str(e)}")

            if months:
                payload = {'months': months}
                cache = DashboardPanelCache.query.filter_by(key='net_savings_series').first()
                if cache is None:
                    cache = DashboardPanelCache(key='net_savings_series', payload_json=json.dumps(payload))
                    db.session.add(cache)
                else:
                    cache.payload_json = json.dumps(payload)
